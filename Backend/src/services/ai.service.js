...
// 🔐 UPDATED: Strict isolated runtime compilation bypass logic for Puppeteer setup
async function generatePdfFromHtml(htmlContent) {
    ...
    // Agar project live Render environment par chal raha hai toh strict absolute storage route lock karo
    if (process.env.PUPPETEER_CACHE_DIR) {
        const path = require("path");
        const fs = require("fs");
        
        // Render dynamic installation standard directories lookup
        ...
        if (fs.existsSync(baseCachePath)) {
            ...
            if (versions.length > 0) {
                // Instantly grabs the dynamically extracted chrome installation matching folder structure
...
async function generateResumePdf({ resume, selfDescription, jobDescription }) {
    try {
        ...
        const prompt = `Generate resume for a candidate with the following details:
                        ...
                        The content of resume should be not sound like it's generated by AI